...
void Base::processPress	(	uint32_t	bNumber	) {
	...
	if ( s->press == false ) {
		...
		if ( s->bounce == true ) {								/// Если идет проверка на дребезг.
			if ( s->bounceTime - this->cfg->taskDelayMs > 0 ) {	/// Если время проверки еще не закончилось (мы от оставшегося времени отнимаем время, прошедшее с предыдущей проверки).
		...
		} else {												/// Если проверка на дребезг уже прошла,
																/// при этом клавишу еще держат.
			...
			if ( s->buttonLongClickTime < p_st->longPressDetectionTimeS * 1000 )/// Если прошло времени меньше,
				return;															/// чем нужно для того, чтобы кнопка
