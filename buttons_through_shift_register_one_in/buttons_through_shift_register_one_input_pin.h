...
private:
	void selectButton			( const uint32_t &b_number );						// Выбрать нужную кнопку через сдвиговом регистре (подать на ее вывод 0, а на остальные 1).
	bool checkPress				( void );											// Метод возвращает true - если клавиша нажата, false, если нет.

	void processPress			( const uint32_t &b_number );						// Обработка клавиши в случае, если она в данный момент нажата.
	void processNotPress		( const uint32_t &b_number );						// Если отпущена.
